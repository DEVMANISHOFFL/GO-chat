...
/** Events received from the server (after ws.ts aliases payload -> data). */
...
/** Events sent from the client to the server (server expects `payload`, not `data`). */
export type ClientEvent =
  | { type: 'ping' }
  | {
      type: 'message.send';
      payload: { tempId: string; roomId: string; content: string };
    }
